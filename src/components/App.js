...
class App extends Component {
  ...
  handleClick(id, url) {
    ...
    let temp = [];
    elements[id]['locked'] === false &&
      elements[id][url] === false &&
      this.setState(
        {
          elements: elements.map(
            (el, i) => (i === id ? { ...el, [url]: true } : el)
          ),
          click: click + 1
        },
        () => {
            ...
            if (temp[0] !== temp[1]) {
              ...
              setTimeout(
                () =>
                  this.setState({
                    click: 0,
                    elements: this.state.elements.map(
                      el =>
                        el[temp[0]] || el[temp[1]]
                          ? { ...el, locked: true }
                          : el
    ...
    return (
      <React.Fragment>
        <StyledApp>
          <h2>Memory Game</h2>
          <ul>
            {elements.map(el => Object.keys(el)[0]).map((url, i) => (
          ...
          {!elements.map(el => el['locked'] === false).includes(true) && (
